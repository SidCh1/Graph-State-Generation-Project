...
def run_single_simulation(N):
    p = 0.05  # or whatever value you want to fix
...
# ------------------------------
def run_simulations():
    Ns = list(range(100, 501, 20))  # Vary N from 100 to 500 with step size 20
    num_runs = 500  # Reduce for testing if needed
    ...
    df_mean.to_csv("simulation_statistics_vs_N.csv", index=False)
    print("Saved results to simulation_statistics_vs_N.csv")
